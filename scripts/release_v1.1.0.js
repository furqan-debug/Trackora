...
function sign(filePath) {
  ...
  // (what you get when you run: tauri signer generate — it prints a base64 key)
  // Our signing.key file IS already that base64 value.
...
try {
  ...
  // 4. Read .sig files (Tauri signer writes them alongside the binary)
  ...
  const latest = {
    version: VERSION,
    notes: `Trackora v${VERSION} — Timezone-aware activity tracking, accurate daily totals, session deduplication, and stability improvements.`,
  ...
  // 7. Done
  console.log('\n\x1b[32m════════════════════════════════════════════════');
  console.log(`  ✅  Trackora v${VERSION} release packaged!`);
  console.log('════════════════════════════════════════════════\x1b[0m');
  ...
  console.log(`  → Then create GitHub release v${VERSION} and upload all files above.`);
