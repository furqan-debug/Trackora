    ...
    return (
        ...
        >
            <div className="relative">
                ...
                <div className="relative w-20 h-20 rounded-[28px] bg-surface border border-border flex items-center justify-center text-text-muted shadow-shell-sm">
                    {icon ?? <Inbox className="w-8 h-8 opacity-40" aria-hidden />}
                </div>
            </div>
            <div className="space-y-2 relative z-10">
                <p className="text-lg font-bold text-text-main tracking-tight leading-none">{title || 'Data Unavailable'}</p>
                {description && (
                    <p className="text-[13px] text-text-muted max-w-sm mx-auto font-medium leading-relaxed opacity-80">{description}</p>
                )}
            </div>
