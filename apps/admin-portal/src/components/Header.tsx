...
import { Star, LogOut, Menu, ChevronDown, User as UserIcon, Search, Command } from 'lucide-react';
...
export function Header({ onOpenMobileMenu }: HeaderProps) {
    ...
    return (
        <header className="h-16 md:h-16 sticky top-0 z-[60] w-full bg-white/80 backdrop-blur-xl border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 transition-all">
            {/* 💎 Brand Architecture */}
            ...
            <div className="flex items-center gap-5 lg:gap-8" ref={menuRef}>
                ...
                {/* Global Command */}
                <div className="hidden lg:flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 py-1.5 w-64 group hover:border-slate-300 transition-all cursor-text shadow-sm">
                    <Search className="w-3.5 h-3.5 text-slate-400 group-hover:text-slate-600 transition-colors" />
                    <span className="flex-1 ml-3 text-[11px] font-black text-slate-300 uppercase tracking-widest group-hover:text-slate-400">Command</span>
                ...
                <div className="flex items-center gap-3">
                    {/* Starred Resources */}
                    <button 
                        onMouseEnter={() => favorites.length > 0 && setShowFavoritesDropdown(true)}
                        className={clsx(
                            "w-10 h-10 rounded-xl border border-slate-200 flex items-center justify-center transition-all bg-white shadow-sm",
                            favorites.length > 0 ? "text-amber-500 border-amber-100 bg-amber-50/20" : "text-slate-400 hover:text-slate-600 hover:border-slate-300"
                        )}
                    >
                        <Star className={clsx("w-4 h-4", favorites.length > 0 && "fill-current")} />
                    </button>

                    {showFavoritesDropdown && (
                        <div 
                            className="absolute top-full right-0 mt-2 w-64 p-3 bg-white border border-slate-200 rounded-2xl shadow-2xl animate-in fade-in slide-in-from-top-2 duration-200 z-50"
                            onMouseLeave={() => setShowFavoritesDropdown(false)}
                        >
                            <div className="px-3 pb-2 mb-2 border-b border-slate-50">
                                <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Marked Resources</span>
                            </div>
                            <div className="space-y-1">
                                {favorites.map((fav) => (
                                    <Link
                                        key={fav.path}
                                        to={fav.path}
                                        className="flex items-center gap-3 px-3 py-2 rounded-lg text-[11px] font-black uppercase tracking-tight text-slate-500 hover:text-slate-900 hover:bg-slate-50 transition-all"
                                    >
