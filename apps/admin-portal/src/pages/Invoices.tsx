    ...
    return (
        <div className="p-8 max-w-[1400px] mx-auto w-full">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-semibold text-text-main tracking-tight">Invoices</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-surface rounded-xl border border-border p-5 shadow-shell-sm">
                    <div className="text-xs font-semibold text-text-muted mb-2">Outstanding Balance</div>
                    <div className="text-3xl font-light text-rose-500">${totalOutstanding.toFixed(2)}</div>
                </div>
                <div className="bg-surface rounded-xl border border-border p-5 shadow-shell-sm">
                    <div className="text-xs font-semibold text-text-muted mb-2">Total Drafts</div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-surface-hover text-text-muted font-medium border-b border-border">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-semibold text-xs ">Issue Date</th>
                                <th className="px-6 py-4 font-semibold text-xs ">Due Date</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-text-muted">Loading invoices...</td>
                                </tr>
                            ) : filtered.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="px-6 py-16 text-center">
                                        <div className="flex flex-col items-center justify-center text-text-muted">
                                            ...
                                            <p className="text-base font-medium text-text-muted">No invoices found</p>
                                ...
                                filtered.map((inv) => (
                                    <tr key={inv.id} className="hover:bg-surface-hover/50 transition-colors group">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="font-semibold text-text-main">{inv.clients?.name || 'Unknown Client'}</span>
                                        </td>
