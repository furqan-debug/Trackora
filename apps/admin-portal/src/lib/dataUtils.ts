...
export function getHubstaffBlocks(samples: any[], targetTz?: string | null): HubstaffBlock[] {
    ...
    samples.forEach(s => {
        ...
        // Derive the localized hour + minute using Intl so we never rely on the
        // Date constructor mis-interpreting a locale-formatted string as UTC.
        ...
        try {
            ...
            localMin = date.getMinutes();
        }

        // Snap to 10-minute block
        const blockStartMin = Math.floor(localMin / 10) * 10;

        // Build a stable key from the UTC timestamp of the 1st sample in this block
        // but keyed by (localHour, blockStartMin) so grouping is in local time.
        const key = `${date.toISOString().substring(0, 10)}_${String(localHour).padStart(2, '0')}:${String(blockStartMin).padStart(2, '0')}`;

        if (!blocks.has(key)) {
            blocks.set(key, { samples: [], localHour, localBlockMin: blockStartMin });
        }
        blocks.get(key)!.samples.push(s);
    });
    ...
    sortedKeys.forEach(key => {
        ...
        const startH = String(localHour).padStart(2, '0');
        const startM = String(localBlockMin).padStart(2, '0');
        const endTotalMin = localHour * 60 + localBlockMin + 10;
