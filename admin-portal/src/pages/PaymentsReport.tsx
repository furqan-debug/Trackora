                    ...
                    <div className="flex bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
                        {['All', 'Completed', 'Pending', 'Failed'].map(s => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${filterStatus === s ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'
                                    }`}
                            >
                                {s}
                            </button>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-50">
                                <th className="pl-10 pr-6 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Recipient</th>
                                <th className="px-6 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Method & Reference</th>
                                ...
                                <th className="pl-6 pr-10 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">Amount</th>
                            </tr>
                        </thead>
                                    ...
                                    <tr key={p.id} className="hover:bg-slate-50/50 transition-all group cursor-pointer">
                                        <td className="pl-10 pr-6 py-5">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 rounded-2xl bg-slate-100 flex items-center justify-center font-bold text-slate-400 text-sm border border-slate-200 group-hover:bg-white group-hover:scale-110 transition-all">
                                                    {p.member.charAt(0)}
                                                </div>
                                                <span className="text-sm font-bold text-slate-800 tracking-tighter">{p.member}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col">
                                                <span className="text-xs font-bold text-slate-600">{p.method}</span>
                                                <span className="text-[10px] text-slate-300 font-bold tracking-widest uppercase">{p.reference}</span>
                                            </div>
                                        </td>
...
function StatusBadge({ status }: { status: Payment['status'] }) {
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ring-1 ${styleMap[status]}`}>
